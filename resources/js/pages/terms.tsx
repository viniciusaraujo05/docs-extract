import React, { useEffect } from 'react';
import { motion } from 'framer-motion';
import { router, usePage } from '@inertiajs/react';
import { useTranslation } from 'react-i18next';
import { FileJson, FileText, Scale, AlertTriangle, CheckCircle, XCircle, Shield, Database, Lock, UserX } from 'lucide-react';
import { Button } from '@/components/ui/button';

export default function Terms() {
    const { t, i18n } = useTranslation();
    const { props } = usePage<{ locale: string }>();
    const locale = props.locale || 'en';
    
    useEffect(() => {
        i18n.changeLanguage(locale);
        localStorage.setItem('selected-locale', locale);
    }, [locale, i18n]);

    return (
        <>
            <div className="min-h-screen bg-background text-foreground">
                <header className="border-b border-border bg-muted0 backdrop-blur-xl sticky top-0 z-50">
                    <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="flex items-center gap-2 cursor-pointer"
                            onClick={() => router.visit(`/${locale}`)}
                        >
                            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center">
                                <img src="/docset.png" alt="Docset Logo" className="w-full h-full object-cover" />
                            </div>
                            <span className="font-bold text-lg">DOCSET</span>
                        </motion.div>
                        <Button
                            variant="outline"
                            onClick={() => router.visit(`/${locale}`)}
                            className="border-border text-foreground hover:bg-muted"
                        >
                            {t('Back to home')}
                        </Button>
                    </div>
                </header>

                <div className="relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-b from-blue-950/20 via-transparent to-transparent" />
                    <div className="absolute inset-0">
                        <motion.div 
                          className="absolute top-1/4 right-1/4 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl"
                          animate={{
                            scale: [1, 1.2, 1],
                            opacity: [0.2, 0.3, 0.2],
                          }}
                          transition={{
                            duration: 10,
                            repeat: Infinity,
                            ease: "easeInOut"
                          }}
                        />
                    </div>

                    <div className="max-w-4xl mx-auto px-6 py-24 relative z-10">
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.8 }}
                        >
                            <div className="mb-12">
                                <div className="flex items-center gap-3 mb-6">
                                    <div className="w-12 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center">
                                        <Scale className="h-6 w-6 text-blue-600" />
                                    </div>
                                    <h1 className="text-5xl font-bold">{t('legal.terms.title')}</h1>
                                </div>
                                <p className="text-xl text-muted-foreground">
                                    {t('legal.terms.last_updated')}: {new Date().toLocaleDateString(locale === 'pt' ? 'pt-BR' : 'en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                                </p>
                            </div>

                            <div className="prose prose-lg max-w-none">
                                <Section
                                    icon={<FileText className="h-5 w-5" />}
                                    title={t('legal.terms.sections.acceptance.title')}
                                    delay={0.1}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.acceptance.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<CheckCircle className="h-5 w-5" />}
                                    title={t('legal.terms.sections.description.title')}
                                    delay={0.2}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.description.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<AlertTriangle className="h-5 w-5" />}
                                    title={t('legal.terms.sections.accuracy.title')}
                                    delay={0.3}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.accuracy.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Scale className="h-5 w-5" />}
                                    title={t('legal.terms.sections.accounts.title')}
                                    delay={0.4}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.accounts.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Shield className="h-5 w-5" />}
                                    title={t('legal.terms.sections.acceptable_use.title')}
                                    delay={0.5}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.acceptable_use.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Database className="h-5 w-5" />}
                                    title={t('legal.terms.sections.content.title')}
                                    delay={0.6}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.content.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<FileText className="h-5 w-5" />}
                                    title={t('legal.terms.sections.pricing.title')}
                                    delay={0.7}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.pricing.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<CheckCircle className="h-5 w-5" />}
                                    title={t('legal.terms.sections.availability.title')}
                                    delay={0.8}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.availability.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<UserX className="h-5 w-5" />}
                                    title={t('legal.terms.sections.termination.title')}
                                    delay={0.9}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.termination.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Scale className="h-5 w-5" />}
                                    title={t('legal.terms.sections.liability.title')}
                                    delay={1.0}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.liability.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Shield className="h-5 w-5" />}
                                    title={t('legal.terms.sections.warranties.title')}
                                    delay={1.1}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.warranties.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Lock className="h-5 w-5" />}
                                    title={t('legal.terms.sections.data_protection.title')}
                                    delay={1.2}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.data_protection.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<FileText className="h-5 w-5" />}
                                    title={t('legal.terms.sections.changes.title')}
                                    delay={1.3}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.changes.content')}
                                    </p>
                                </Section>

                                <Section
                                    icon={<Scale className="h-5 w-5" />}
                                    title={t('legal.terms.sections.governing.title')}
                                    delay={1.4}
                                >
                                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                                        {t('legal.terms.sections.governing.content')}
                                    </p>
                                </Section>

                                <div className="mt-12 p-6 bg-muted border border-border rounded-2xl">
                                    <h3 className="text-xl font-semibold mb-3">{t('legal.terms.sections.contact.title')}</h3>
                                    <p className="text-muted-foreground leading-relaxed">
                                        {t('legal.terms.sections.contact.text')}
                                    </p>
                                    <p className="text-blue-600 mt-2">
                                        <a href="mailto:legal@docset.app" className="hover:text-blue-700">
                                            legal@docset.app
                                        </a>
                                    </p>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                </div>

                <footer className="border-t border-border py-8 mt-24">
                    <div className="max-w-4xl mx-auto px-6 text-center text-sm text-muted-foreground">
                        <p>© {new Date().getFullYear()} DOCSET. All rights reserved.</p>
                    </div>
                </footer>
            </div>
        </>
    );
}

function Section({ 
    icon, 
    title, 
    children, 
    delay 
}: { 
    icon: React.ReactNode; 
    title: string; 
    children: React.ReactNode; 
    delay: number;
}) {
    return (
        <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay, duration: 0.5 }}
            className="mb-12"
        >
            <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-600">
                    {icon}
                </div>
                <h2 className="text-2xl font-bold">{title}</h2>
            </div>
            <div className="ml-13">
                {children}
            </div>
        </motion.div>
    );
}
